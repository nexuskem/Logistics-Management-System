...
const Expenses = () => {
  ...
  const [formDataExpense, setFormDataExpense] = useState({ date: '', category: 'MAINTENANCE', vehicle_id: '', amount: '', notes: '' });
  const [formDataFuel, setFormDataFuel] = useState({ date: '', vehicle_id: '', station: '', litres: '', total: '' });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-brand-panel rounded-2xl shadow-xl border border-brand-border overflow-hidden">
        <div className="border-b border-brand-border flex bg-brand-panel-light/30">
          <button 
            className={`px-8 py-5 font-medium text-sm transition-colors relative ${activeTab === 'expenses' ? 'text-brand-orange' : 'text-brand-muted hover:text-white'}`}
          ...
          <button 
            className={`px-8 py-5 font-medium text-sm transition-colors relative ${activeTab === 'fuel' ? 'text-brand-orange' : 'text-brand-muted hover:text-white'}`}
               ...
               {loading ? <tr><td colSpan="5" className="p-8 text-center text-brand-muted">Loading...</td></tr> : expenses.map(exp => (
                 <tr key={exp.id} className="hover:bg-brand-panel-light/60 transition-colors">
                   ...
                   <td className="px-6 py-5">
                     <span className="px-3 py-1.5 bg-brand-panel-light text-brand-muted border border-brand-border rounded-md text-xs font-bold">{exp.category}</span>
                   </td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setFormDataExpense({ date: '', category: 'MAINTENANCE', vehicle_id: '', amount: '', notes: '' }); setFormDataFuel({ date: '', vehicle_id: '', station: '', litres: '', total: '' }); }} title={activeTab === 'expenses' ? "Record General Expense" : "Record Fuel Intake"}>
        {activeTab === 'expenses' ? (
          <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); }}>
            ...
            <div className="pt-2 flex justify-end gap-3 mt-4 border-t border-brand-border/50">
              ...
              <button type="submit" className="px-5 py-2.5 rounded-lg text-sm font-bold bg-brand-orange text-white hover:bg-orange-600 shadow-lg shadow-orange-500/30 transition-all">Save Expense</button>
            </div>
          </form>
        ) : (
          <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); }}>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                   <div className="bg-brand-panel-light p-4 rounded-xl border border-brand-border mt-2 flex justify-between items-center text-white">
                     <span className="font-bold">Total Cost (KES):</span>
                     <input type="number" value={formDataFuel.total} onChange={e => setFormDataFuel({...formDataFuel, total: e.target.value})} className="w-32 px-4 py-1.5 bg-brand-dark border border-brand-border rounded-md text-brand-orange font-bold text-lg text-right focus:ring-2 focus:ring-brand-orange outline-none" placeholder="10500" required />
                  </div>
                </div>
              </div>
            </div>
            <div className="pt-2 flex justify-end gap-3 mt-4 border-t border-brand-border/50">
              <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 rounded-lg text-sm font-bold text-brand-muted hover:text-white transition-colors">Cancel</button>
              <button type="submit" className="px-5 py-2.5 rounded-lg text-sm font-bold bg-brand-orange text-white hover:bg-orange-600 shadow-lg shadow-orange-500/30 transition-all">Save Fuel Log</button>
            </div>
          </form>
        )}
      </Modal>
